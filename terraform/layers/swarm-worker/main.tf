...
resource "digitalocean_tag" "role_swarm_worker" {
  name = "${var.project}:${var.environment}:ansible_role:swarm_worker"
}

module "swarm_workers" {
  source = "../../modules/worker_droplet"
  droplet_name = "${var.project}-${var.environment}-swarm-worker"
  count = 3 
  project = "${var.project}"
  ...
  tags = [
    "${digitalocean_tag.role_swarm_worker.id}",
  ]
  ...
  ansible_tarball = {
    ...
    role = "${digitalocean_tag.role_swarm_worker.name}"
  }
}
